...
module.exports = {
    ...
    Headline.find({}).then(function(data) {
      var olderThanTwelve =
        ((new Date().getTime() / 1000 / 3600) * 60 -
          (Date.parse(data[0].updatedAt) / 1000 / 3600) * 60) /
          60 >
        12;
      if (olderThanTwelve) {
        Headline.destroy().exec(function(err) {
          if (err) {
            ...
            console.log("deleted");
            request("http://www.fantasypros.com/mlb/player-news.php", function(
              error,
              response,
              data
            ) {
  ...
  getUserPlayerSpecifcNews: function(req, res) {
    ...
    Headline.find({ body: { contains: playerName } }).exec(function(err, data) {
      res.send(data);
    });
  }
};
