...
module.exports = {
  getPlayerStats: function(req, res) {
    var playerName = req.query.name,
      url = req.query.url,
      player;
    request("http://www.fantasypros.com" + url, function(
      error,
      response,
      data
    ) {
      ...
      var $ = cheerio.load(data),
        statsContainer = $(".feature-stretch table tbody tr")
          .last()
          .children();
      headshotImg = $(".side-nav img")[0].attribs.src;
      Playerdb.find({ name: playerName }).then(function(response) {
        player = response[0];
        ...
        Playerdb.update({ id: player.id }, player).exec(function(err, player) {
          res.send(player[0]);
        });
      });
    });
